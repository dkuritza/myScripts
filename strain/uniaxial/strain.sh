...
strain_grabb (){
  ...
  if [ ${axis} = x ]; then
    if [ ${epsy} = 0 ]; then
      ...
      poissonRatio=`echo "-(${epsx}/${epsy})" | bc -l`
    fi
    strain=${epsx}
    else
    if [ ${epsx} = 0 ]; then
      ...
      poissonRatio=`echo "-(${epsy}/${epsx})" | bc -l`
    fi
    strain=${epsy}
  fi
}
...
data_printer (){
  printf "%5.2f %10.8f %10.8f %24.20f %24.20f %12.8f %8.4f %8.4f %8.4f\n" $1 ${parxd} ${paryd} ${strain} ${poissonRatio} ${energy} ${e1} ${e2} ${gap} >> ../$2.dat
}
...
run_function (){
  ...
  energy_grabb
  gap_grabb
  data_printer ${per} ${data}
  (head -n 1 ../${data}.dat && tail -n +2 ../${data}.dat | sort -k 1 -n) > ../${data}.temp
  mv ../${data}.temp ../${data}.dat
}
...
for axis in ${DIRECTION}; do
  ...
  if [ -s ${data}.dat ]; then
    printf "${data}.dat already exists! Appending data...\n\n"
  else
	  printf "%5s %10s %10s %24s %24s %12s %8s %8s %8s\n" % x y Strain PoissonRatio Energy HOMO LUMO gap >> ${data}.dat
  fi
  for per in $(seq ${NMIN} ${STEP} ${NMAX}); do
    printf "Running ${per}%% ${axis}-strain: "
    if [ -d ./${per}/ ]; then
